...
/* SelectListWidget :: [{k:a, v: { label: String, options: [DOM], value: a }]
 *
 * Create a list of select drop-downs. The SelectListWidget constructor takes
 * a single list argument, where each object has a "k" attribute and a "v"
 * attribute. The "v" attribute has three parts: label, which appears to the
 * right of the drop-down options, a list of OPTIONS to show in the drop-down
 * value, which, if it equals the value of one of options, preselected
 *
 * The value of the entire widget is a list of values representing the
 * currently-selected values
 */
function SelectListWidget(entries) { Widget.apply(this); var widg = this;

	var selvals = {};
	var inputElems = [];

	map(function(entry) { selvals[entry.k] = entry.v.value; }, entries);
	var evts = [];
	this.dom = UL({
    ...
    style: {'list-style': 'none'}
		...
		var sel = SELECT({
      style: {'margin-right': '1em', 'border': '2px solid gray'},
      name:'slistwidget-'+startnum
    }, entry.v.options);
		inputElems.push(sel);
		evts.push($E(sel).transform_e(function(_) {
      return {k:entry.k,v:_};
    }));
		return LI(sel,entry.v.label);
	}, entries));

	this.behaviors.value = merge_e.apply({},evts).transform_e(function(ne) {
		selvals[ne.k] = ne.v;
		return selvals;
	}).startsWith(selvals).transform_b(function(sels) {
    return fold(function(v, acc) {
      if (sels[v.k]) acc[v.k] = sels[v.k];
      return acc;
    },{},entries);
  });
	this.behaviors.inputElems = constant_b(inputElems);
...
inheritFrom(SelectListWidget,InputWidget);
      ...
      if (customHeader) {
        ...
        var indicatorB = colB.lift_b(function(colOrder) {
          return (colOrder.length > 0 && colOrder[0].col == thisIndex)
            ? (sortOrder == 1 ? IMG({src: "/static/images/bullet_arrow_up.png"}) 
                              : IMG({src: "/static/images/bullet_arrow_down.png"}))
            : "";
        });
...
function iframeLoad_e(iid,exceptsE) {
	...
	var lv = extractEvent_e(theIframe,'load')
      ...
      return !location.match(/static\/blank\.html$/);
    })
		.transform_e(function(_) {
			var child = window.frames[iid].document.body.childNodes[0];
			window.frames[iid].location = '/static/blank.html'; 
                        if(child) {
			  return child.innerHTML.parseJSON();
                        } else {
                          return {value: {error: 'Empty iframe', empty: true}};
                        }
		});
