...
var CAP_EXPORTS = (function() {
  ...
  var isSameOrigin = function(url) {
    var protocol_host_port = window.location.protocol + '//' +
        window.location.host + window.location.port;
    log('PHP: ' + protocol_host_port);
    log('URL: ' + url);
    return url.indexOf(protocol_host_port) === 0;
  };


  var log = function(str) {
    window.console ? console.log(str) : null;
  };
  var xDomainAJAX = function(url, method, data, success, failure) {
    log('Trying XDR: ' + url);
    var xdr = new window.XDomainRequest();
    var onerror = function(e) {
      failure({ status: 500, message: 'XDR failure' + e });
    };
    var ontimeout = function(e) {
      failure({ status: 500, message: 'XDR timeout' + e });
    };
    var onprogress = function(e) {
      log('Progressing through XDR');
    };
    var onload = function() {
      log('XDR onload: ' + xdr.responseText);
  ...
  var makeAsyncAJAX = function(url, method, data, success, failure) {
     if (!isSameOrigin(url) && window.XDomainRequest) {
       xDomainAJAX(url, method, data, success, failure);
       return;
     }
  ...
  Capability.prototype.invoke = function(method, data, success, failure) {
    ...
    var wrappedSuccess = function(result) {
      ...
      if (success) {
        ...
        console.log('Calling success w/result: ', success.toString());
  ...
  CapServer.prototype.dataPreProcess = function(w) {
    ...
    var ret = JSON.stringify({ value: w }, function(k, v) {
  ...
  CapServer.prototype.dataPostProcess = function(w) {
    ...
    var me = this;
    console.log('Post-processing: ', w);
    var ret = JSON.parse(w, function(k, v) {
      try {
        var k = Object.keys(v);
        console.log('k[0]: ', k[0]);
        if (k.length === 1 && k[0] === '@') {
          console.log('Restoring: ', v['@']);
          return me.restore(v['@']);
        }
      }
      catch (e) { }
    ...
    console.log('Post-processed: ', ret);
    return ret;
  };
