...
/*
  getEE : EventStream[Capability[get: -> α]] ->
          { success: EventStream[α],
            error: EventStream[α] }
*/
function getEE(capE) {
  var success_e = receiver_e();
  var error_e = receiver_e();
  capE.transform_e(function(cap) {
    cap.get(function(result) { success_e.sendEvent(result); },
            function(err) { error_e.sendEvent(err) });
  });
  return { success: success_e, error: error_e };
}
...
function capStreamWSO_e(objE, capE, verifier) {
  var cap;
  ...
  capE.transform_e(function(c) {
    console.log('updating cap in capStreamWSO_e to: ', c);
    cap = c;
  });
...
function belayGetWSO_e(objE, cap, verifier) {
  return capStreamWSO_e(objE, objE.constant_e(cap), verifier);
}

function makeServerSaving(responseMethod) {
  return function(toReqFn, replaceValue, cap, verifier) {
    var objE = this.behaviors.value.changes().calm_e(500).transform_e(toReqFn)
    if(typeof cap === 'function') {
      var cap = this.behaviors.value.changes().calm_e(400).transform_e(cap);
    }
    this.events.serverResponse = responseMethod(objE, cap, verifier);
...
InputWidget.prototype.belayServerSaving = makeServerSaving(belayGetWSO_e);

InputWidget.prototype.capStreamServerSaving = makeServerSaving(capStreamWSO_e);

InputWidget.prototype.belayDraftSaving = function(reqEvt,toReqFn, cap) {
  this.events.serverResponse = belayGetWSO_e(combine_eb(toReqFn,reqEvt,this.behaviors.value), cap);
