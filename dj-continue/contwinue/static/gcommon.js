...
function ToggleWidget(ontxt,offtxt) {
	Widget.apply(this);
	var onlink = A({href:'javascript://Show'},ontxt);
	var offlink = A({href:'javascript://Hide'},offtxt);
...
function LinkListWidget(values,initial,prevnext) {
	...
	function makeChooser(idx,k,v) {
		var d = A({href:'javascript://Choose'},v);
	...
	if(prevnext) {
		...
		this.makePrevBtn = function() {
			var p = A({href:'javascript://Choose'});
		...
		this.makeNextBtn = function() {
			var n = A({href:'javascript://Choose'});
...
function LinkWidget(text,value) {
	ClickWidget.apply(this,[A({href:'javascript://Click'},text),value]);
}
...
InputWidget.prototype.withButton = function(bwidg,domfn) {
	return new ButtonInputWidget([this],{value:bwidg},function(a) {return a;},function(a,b) {return domfn(a[0],b.value);});
}	
...
function Column(className,headerText,sortfn,makeTD) {
...
function HeaderSortWidget(columns,initSort) {
	...
	for(i=0; i<columns.length; i++) {
		if(columns[i].sortFn) {
			var hl = A({href:'javascript://Sort',className:'k-url',title:'Sort by '+columns[i].headerText},columns[i].headerText);
			hl.onclick = (function(k) {return function() {headerClicksE.sendEvent(k);};})(i);
		}
		else {
			hl = columns[i].headerText;
		}
		tableHeadCols.push(TH({className:columns[i].className,scope:'col'},hl));
		if(columns[i].defaultSort) defaultCol = i;
	}

	if(!initSort) initSort = [{col:defaultCol,order:1}];
	this.behaviors.cols = collect_b(initSort,headerClicksE,
		function(newcol,cols) {
			if(newcol == defaultCol)
				return cols[0].col == defaultCol ?
					[{col:defaultCol,order:cols[0].order*-1}] : 
					[{col:defaultCol,order:1}];
			else {
	...
	this.dom = THEAD(TR(tableHeadCols));
}
inheritFrom(HeaderSortWidget,Widget);

function TableWidget(papersB,classPrefix,columns,filterWidget,tableTitle) {
	Widget.apply(this);
	if(!tableTitle) tableTitle = '';

	if(!filterWidget)
		filterWidget = {dom:'',behaviors:{filterFn:constant_b(function(){return true;})}};
	...
	},papersB,sorter.behaviors.sortFn,filterWidget.behaviors.filterFn);

	var numPapersB = this.behaviors.showPapers.transform_b(function(sp) {return fold(function(v, acc) {return acc+1;},0,sp);});
	
	var paginator = this.makePaginator(numPapersB,classPrefix+'-list');

	this.behaviors.cols = sorter.behaviors.cols;
	this.dom = this.makeDom(filterWidget,tableTitle,paginator,sorter,classPrefix,this.behaviors.showPapers);
}
...
TableWidget.prototype.makeDom = function(filterWidget,ttl,paginator,sorter,classPrefix,showPapersB) {
...
function iframeLoad_e(iid,exceptsE) {
	var theIframe = getObj(iid);
	var lv = extractEvent_e(theIframe,'load')
		.filter_e(function(_) {return window.frames[iid].location != 'about:blank';})
		.transform_e(function(_) {
			var a = window.frames[iid].document.body.childNodes[0].innerHTML;
			window.frames[iid].location = 'about:blank'; 
			return a.parseJSON();
		});
